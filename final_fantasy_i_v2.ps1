...
function grind ([int]$time = 30){
    ...
    for ($i = 0; $i -lt $time; $i++){
        ...
        while (-not (inBattleSoloMonk)){
            pressKey ($KEYMAP.l, $KEYMAP.r) |Out-Null
            sleep 0.1
        }
        while (inBattleSoloMonk){
            sleep -s 5;
            echo "Confirm sent: $remaining"
            press_key $KEYMAP.confirm
        }
        #echo "Ending $i"
    }
...
function Get-PixelColor {
    # Perhaps separate the construction and destruction calls from the use
    # as this will be called mlutiple times. 
    param (
        [int]$x = 0,
        [int]$y = 0
    )
...
function pressKey {   # Am I doing to much???
    <#
        When press key is called, it takes a list of one or more 
        commands then performs them in order. It is wrapped by safety 
        so it will construct wshell if needed, or use currently 
        existing wshell.
    #>
    param(
        [string[]]$keyList = @()
    ...
    foreach ($key in $keyList){
        $global:wshell.AppActivate($TITLE) #I only need to set focus if it has lost focus. Is it posisible to check?
        Sleep $DELAY
